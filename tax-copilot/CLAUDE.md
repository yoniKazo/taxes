...
- **2026-08-29** — ריצת Task 6 (diagnostics) על `mh6`/`s6` נתקלה **שוב** ב-`OSError: The paging file is too small` (כבר תועד קודם ב-X4, 2026-08-23-ish, בהקשר אחר) -- הפעם תוך כדי טעינת torch/faiss בתהליך Python חדש **בזמן שה-uvicorn dev server רץ ברקע** עם עותק נפרד כבר-טעון של אותה ערימה (torch/sentence-transformers/faiss, ~500MB+). נימוק: כל תהליך Python חדש שמייבא `tools.py`→`build_index.py` טוען את הערימה הכבדה מחדש; שני תהליכים כאלה בו-זמנית על מכונה עם מעט זיכרון פנוי חוצים את גודל ה-pagefile הקבוע. תיקון (עוקף, לא פותר את הגודל הקבוע עצמו): לעצור את שרת ה-uvicorn לפני הרצת סקריפט אבחון חד-פעמי שטוען את אותה ערימה, להריץ, ואז להפעיל מחדש. **סימן מזהה נלווה שהתגלה אגב כך:** `total_wall_ms` חריג (מאות אלפי ms) יחד עם `tool_outputs=""` ריק ב-trace הוא סימן טוב לבעיית תשתית (טעינת מודל/רשת תקועה), לא לכשל reasoning -- ה-tracer לא מספיק לרשום אף שלב לפני שחריגת ה-timeout יורה, כך שהעדר תוכן-שלב עצמו הוא הרמז.
